//
//  QRefreshControl.swift
//  q
//
//  Created by qli on 2017/3/4.
//  Copyright © 2017年 qli. All rights reserved.
//

import UIKit

class QRefreshControl: UIControl {
    ...
    ///上拉多少距离开始加载更多
    internal var loadMoreSpace:CGFloat = 2
    ...
    //顶部偏移
    fileprivate var topOffset:CGFloat = -64
    //我是有底线的
    fileprivate var morelable: UILabel!
    
    //是否更多加载完成
    fileprivate var isMore = true
    ...
    //加载结束 记得调用该方法
    internal func endLoadingmore(_ isb: Bool) -> Void{
            self.isMore = isb
    ...
    fileprivate func loadmoreViewAdd() -> Void{
        ...
        self.morelable = UILabel()
        self.morelable.textAlignment = NSTextAlignment.center
        self.morelable.isHidden = true
        self.morelable.font = UIFont.boldSystemFont(ofSize: 12)
        self.morelable.backgroundColor = UIColor.clear
        self.morelable.text = "—————— 我是有底线的 ——————"
        self.morelable.textColor = UIColor.darkText
        self.morelable.frame = CGRect(x: 0, y: self.scrollView.frame.size.height, width: self.scrollView.frame.size.width, height: 60)
//
        ...
        self.loadmoreActivity.frame =  CGRect(x: 0, y: self.scrollView.frame.size.height, width: 36, height: 36)
        self.loadmoreActivity.alpha = 0
        self.loadmoreActivity.backgroundColor = UIColor.gray
...
//        self.scrollView.sendSubview(toBack: self.morelable)
        //给加载更多View留位置
    ...
    fileprivate func moreLine(){
        self.morelable.isHidden = false
        let contentSizeHeight = self.scrollView.contentSize.height
        self.morelable.center = CGPoint(x: self.scrollView.frame.width/2, y: contentSizeHeight + 30)
    }
    ...
    fileprivate func loadmoreShow() -> Void{
            ...
            let contentSizeHeight = self.scrollView.contentSize.height
            self.loadmoreActivity.center = CGPoint(x: self.scrollView.frame.width/2, y: contentSizeHeight + 30)
...
//                self.scrollView.contentInset.bottom += self.showViewHeight
    ...
    override func observeValue(forKeyPath keyPath: String?, of ofObject: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) -> Void{
        ...
        //--------------------------加载更多--------------------------------------------
        if(self.loadmore && (!self.isAnimating()) && self.isMore){
